...
//   dates:          200 entries (per event — covers ~4 yrs of weekly dates)
...
const ruleSchema = z.object({
  pattern:      z.enum(['daily','weekly','monthly_week','monthly_date','annual','weekdays']).optional(),
...
const eventSchema = z.object({
  ...
  time_end:       z.string().max(8).optional(),
  all_day:        z.boolean().optional(),
  dates:          z.array(dateEntrySchema).max(200).optional(),
...
// B-QA6: A password consisting of / padded with spaces was silently accepted
// with no feedback (e.g. "        " passes min(8) with zero visible content).
// Applied only where a NEW password is being set (register, reset) — never on
// login, so an account that registered before this check shipped can still
// sign in with whatever password is already hashed in the DB.
const newPasswordSchema = z.string().min(8).max(200)
  .refine(val => val.trim().length > 0, { message: 'Password cannot be only spaces' })
  .refine(val => val.trim() === val, { message: 'Password cannot start or end with a space' });
